...
module mbist (
    ...
    wire [1:0] max_step = (state == S_M0) ? 2'd0 :
                          (state == S_M1 || state == S_M2) ? 2'd2 : 
                          2'd0;

    wire step_done = (step == max_step);
    
    wire addr_done = (state == S_M2) ? (addr == {`ADDR_W{1'b0}}) : (addr == {`ADDR_W{1'b1}});

    wire [`ADDR_W-1:0] next_addr = 
        (state == S_IDLE)                         ? {`ADDR_W{1'b0}} :
        (state == S_M0)                           ? addr + 1'b1 :
        (state == S_M1 && step_done && addr_done) ? addr : 
        (state == S_M1 && step_done)              ? addr + 1'b1 :
        (state == S_M2 && step_done && addr_done) ? addr :
    ...
    wire write_bit = (state == S_M1);
    assign mem_wdata = {`DATA_W{write_bit}}; 
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            check_en      <= read_en; // Активация проверки данных, если в прошлом такте было чтение
